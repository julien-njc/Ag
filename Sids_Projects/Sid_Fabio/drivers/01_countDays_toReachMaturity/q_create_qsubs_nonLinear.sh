...
do
  ...
  do
    ...
    do
      ...
      do
        cp q_countDaysMaturity_temp_nonLinear.sh  ./qsubs/q_countDaysMaturity_nonLinear$outer.sh
        sed -i s/outer/"$outer"/g                     ./qsubs/q_countDaysMaturity_nonLinear$outer.sh
        sed -i s/veg_type/"$veg_type"/g               ./qsubs/q_countDaysMaturity_nonLinear$outer.sh
        sed -i s/model_type/"$model_type"/g           ./qsubs/q_countDaysMaturity_nonLinear$outer.sh
        sed -i s/start_doy/"$start_doy"/g             ./qsubs/q_countDaysMaturity_nonLinear$outer.sh
        sed -i s/param_type/"$param_type"/g           ./qsubs/q_countDaysMaturity_nonLinear$outer.sh
        let "outer+=1"
      done
    done
  done
