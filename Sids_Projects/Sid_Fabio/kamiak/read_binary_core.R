...
read_binary <- function(file_path, data_type, no_vars){
  ###
  ### Future projections on Aeolus have 4 variables on Kamiak have 8.
  ### Be careful... if you are lucky to read this!!!!
  ### It is a mess!
  if (no_vars == 8){
    if(data_type=="observed") {
      start_year <- 1979
      end_year <- 2016
    } else if (data_type=="modeled_hist"){
      start_year <- 1950
      end_year <- 2005
    } else{
      start_year <- 2006
    ...
    ### This is modeled version on Aeolus.
     if (data_type=="modeled_hist"){
       start_year <- 1950
       end_year <- 2005
       } else {
          start_year <- 2006
...
create_ymdvalues <- function(data_start_year, data_end_year){
  ...
  for (i in 1:nYears){
    ly <- chron::leap.year(Years[i]) # lubridate::leap_year(Years[i]) lubridate is under R 3.5.3.
    if (ly == TRUE){
