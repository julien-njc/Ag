...
output_dir = "/data/hydro/users/Hossein/remote_sensing/05_01_allFields_SeasonCounts/"
...
needed_columns = ["ID", "Acres", "ExctAcr", "county", "CropTyp", "DataSrc", "Irrigtn", "LstSrvD", "Notes", 
                  "RtCrpTy", "image_year", "SF_year", "season_count"]

output_columns = needed_columns.copy()
output_columns.append("SG_params")

limited_columns = ["county", "CropTyp", "SG_params", "image_year"]
...
for seos in sos_list:
    ...
    filename = output_dir + "extended_all_fields_seasonCounts_noFilter_SEOS" + str(seos) + ".csv"
    ...
    output_dataframe.sort_values(by=['image_year', 'county','CropTyp', 'SG_params'], inplace=True)

    filename = output_dir + "all_fields_seasonCounts_noFilter_SEOS" + str(seos) + ".csv"
    output_dataframe.to_csv(filename, index = False)    
