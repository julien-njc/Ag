...
by_year <- T
by_year <- F # for 'obs_hist' on all grids
stats_by_loc <- NA
data_dir <- 'data/Prosser vs Cascades/'
data_dir <- 'data/WA_gridmet/'  # for 'obs_hist' on all grids
#for (model in list.dirs(data_dir, full.names = F, recursive = F)) {
for (model in c('obs_hist')) {    # for 'obs_hist' on all grids
  ...
  for (scenario in c('rcp45', 'rcp85')) {
    ...
    if (model == 'obs_hist' & scenario == 'rcp85')
      next
    data_subdir <- data_dir      # for 'obs_hist' on all grids
    for (loc_file_name in list.files(data_subdir)) {
      ...
      if (by_year) {
        loc_stats <- loc_by_year[-c(3, 5)]
        loc_stats$lat <- loc_lat
        loc_stats$long <- loc_long
        loc_stats$model <- model
        loc_stats$scenario <- ifelse(max(loc$CH_year) == 2100, scenario, NA)
      } else {
        # Compile statistics for each time frame from loc_by_year.
        loc_stats <- data.frame(
          lat = character(), long = character(), model = character(), scenario = character(), range = character(), DD_h = numeric(),
        ...
        if (max(loc$CH_year) == 2100) {
          for (CH_year_from in c(2025, 2050, 2075)) {
            ...
            loc_stats[nrow(loc_stats) + 1, ] <- list(
              loc_lat, loc_long, model, scenario, paste(CH_year_from, CH_year_to, sep = '-'),
          ...
          loc_stats[1, ] <- list(
            loc_lat, loc_long, model, NA, paste(min(loc_by_year$CH_year), max(loc_by_year$CH_year), sep = '-'),
...
write.csv(stats_by_loc, 'data/obs_hist_stats_by_loc.csv', row.names = F)  # for 'obs_hist' on all grids
