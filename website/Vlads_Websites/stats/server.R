...
shinyServer(function(input, output, session) {
  render_map <- function(base_map, grape_variety, model, time_frame, scenario, col, DDh, is_percent, leg_title, palette) {
    ...
    if (model != obs_hist)# & scenario != na & time_frame != '1979–2020')
    ...
    if (sum(mask) > 0) {
      ...
      addLegend(position="bottomleft", pal=pal, values=df$val, title=leg_title, labFormat=lab_format, opacity=1) %>%
  ...
  render_map_wrapper <- function(base_map, grp_vrty, m_tf_s, col, DDh, is_percent, leg_title, palette=c('white', 'red')) {
  ...
  observeEvent(input$nav, {
    if(input$nav == "CDI" & !rendered$CDI){
      ...
      render_map_wrapper(leafletProxy("map_switch_day"), 
                         input$grape_variety_switch_day,
                         extract_scope(input$scope_switch_day, input$model_switch_day), 
                         'switch_day', input$DDh_switch_day, 
                         F, 'days since Sep-01')
...
make_base_map <- function(map_tile) {
  if (map_tile == "ws"){
    ...
    #             addMapPane(name = "maplabels", zIndex = 410) %>% 
    ...
    base_map <- leaflet() %>% 
                addProviderTiles(providers$OpenTopoMap) #, options=tile_opts
  } 
