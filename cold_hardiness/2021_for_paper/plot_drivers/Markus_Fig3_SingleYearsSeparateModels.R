...
output_dir <- "/Users/hn/Documents/01_research_data/cold_hardiness/hossein_locations/Markus_Figrue_3/separateYearsModels/"
...
for (a_location in unique(CH_dt$location)){
  for (a_mission in unique(CH_dt$emission)){
    ...
    a_plt <- ggplot(data=curr_CH) +
             geom_ribbon(aes(x=CH_DoY, ymax=t_max, ymin=t_min), fill="dodgerblue2") + # , alpha=.2
             ...
             facet_grid(. ~ model ~ hardiness_year, scales = "free") + 
             the_theme
    ...
    a_plt_med <- ggplot(data=curr_CH_median) +
                 geom_ribbon( aes(x=CH_DoY, ymax=median_tmax, ymin=median_tmin), fill="dodgerblue") + # , alpha=.2
                 ...
                 # labs(y = "predicted CH                          temperature") + 
                 # ggtitle(paste0(unique(weather$location), ", ", unique(CH_dt$emission), ", ", unique(CH_dt$time_period))) +
                 facet_grid(. ~ hardiness_year, scales = "free") + 
                 the_theme
