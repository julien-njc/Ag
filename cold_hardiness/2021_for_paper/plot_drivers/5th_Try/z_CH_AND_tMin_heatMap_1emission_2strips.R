...
output_dir <- paste0("/Users/hn/Documents/01_research_data/cold_hardiness/hossein_locations/Markus_Figrue_3/5th_try/", 
                     "heatMaps/separateLocs/1emission_2strips/")
...
CH_dt <- CH_dt %>% filter(CH_DoY<=257) %>% data.table()
...
for (a_fucking_emission in unique(CH_dt$emission)){
  for (a_fucking_loc in unique(CH_dt$location)){
    curr <- CH_dt %>% filter(location==a_fucking_loc & emission==a_fucking_emission) %>% data.table()

    ch_plt <- ggplot(curr, aes(CH_DoY, hardiness_year, fill=predicted_Hc)) + 
                geom_tile() + 
                ...
                ylab("year") + labs(fill="cold hardiness") + 
                the_theme + 
                scale_fill_viridis(discrete=FALSE) # scale_fill_gradient() or scale_fill_distiller() or scale_fill_viridis(discrete=FALSE)

    tmin_plt <- ggplot(curr, aes(CH_DoY, hardiness_year, fill=t_min)) + 
                geom_tile() + 
                facet_grid(. ~ emission ~ facet, scale="free") + 
                scale_x_continuous(breaks=CH_DoY$day_count_since_sept, labels=CH_DoY$letter_day) +
                scale_y_continuous(breaks=seq(from = 1980, to = 2050, by = 10) ) + 
                ylab("") + labs(fill=expression(t[min])) + 
                the_theme + 
                scale_fill_viridis(discrete=FALSE) # scale_fill_gradient() or scale_fill_distiller() or scale_fill_viridis(discrete=FALSE)


    plt <- ggarrange(plotlist = list(ch_plt,
                                     tmin_plt),
                          ncol = 2, nrow = 1,
                          legend = "bottom",
                          common.legend = FALSE)
    ...
    ggsave(plot = plt,
           filename=paste0(a_fucking_loc, "_", a_fucking_emission, "_2025_2049_CH_and_tMin_Heat_", "scale_fill_viridis", ".pdf"),
