...
#
CH_dt <- CH_dt %>%
         filter(hardiness_year %in% c(1980:2015, 2025, 2030, 2035, 2040, 2045)) %>%
         data.table()
...
CH_dt_hist_med <- CH_dt_hist %>%
                  group_by(location, emission, variety, CH_DoY)%>% 
                  summarise(median_tmin=median(t_min), median_tmax=median(t_max), median_HC = median(predicted_Hc)) %>%
                  data.table()
CH_dt_hist_med$hardiness_year <- "Observed (median)"


CH_dt_F_med <- CH_dt_F %>%
               group_by(location, emission, variety, hardiness_year, CH_DoY)%>% 
               summarise(median_tmin=median(t_min), median_tmax=median(t_max), median_HC = median(predicted_Hc)) %>%
               data.table()
...
CH_dt_F_med$emission <- factor(CH_dt_F_med$emission, 
                               levels =c("RCP 8.5", "RCP 4.5"), 
                               order=TRUE)

CH_dt_hist_med$emission <- factor(CH_dt_hist_med$emission, 
                                  levels =c("RCP 8.5", "RCP 4.5"), 
                                   order=TRUE)

CH_dt_median <- rbind(CH_dt_hist_med, CH_dt_F_med)

CH_dt_median$hardiness_year <- factor(CH_dt_median$hardiness_year, 
                                      levels =c("Observed (median)", "2025", "2030", "2035", "2040", "2045"), 
                                      order=TRUE)


CH_dt_median$location <- factor(CH_dt_median$location, 
                                levels = sort(unique(CH_dt_median$location)), 
                                order=TRUE)
...
ggsave(plot = a_plt,
       filename = paste0("separateYears_BCC_obser.pdf"),
