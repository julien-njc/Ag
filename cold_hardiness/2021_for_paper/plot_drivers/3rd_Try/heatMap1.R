...
a_plt <- ggplot(CH_dt, aes(CH_DoY, hardiness_year, fill= predicted_Hc)) + 
         ...
         the_theme + 
         scale_fill_distiller() # scale_fill_gradient() or scale_fill_distiller() or scale_fill_viridis(discrete=FALSE)

fig_W <- 15
fig_H <- 10
ggsave(plot = a_plt,
       filename=paste0("3rdTry_Heat_", "scale_fill_distiller", ".pdf"),
