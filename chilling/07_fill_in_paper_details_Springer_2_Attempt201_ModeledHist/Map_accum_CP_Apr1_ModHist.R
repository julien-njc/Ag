...
stats_comp <- lapply(the_dir_stats, read.table, header = T, as.is=T, stringsAsFactors=FALSE) 
...
stats_comp_modeledHist <- stats_comp %>% filter(model != "observed")
stats_comp_modeledHist <- stats_comp_modeledHist %>% filter(time_period != "2005_2024")


stats_comp_F <- stats_comp %>% filter(time_period %in% c("2025_2050", "2051_2075", "2076_2100"))
stats_comp <- rbind(stats_comp_modeledHist, stats_comp_F)
rm(stats_comp_F, stats_comp_modeledHist)
########################################################################
#
#    Edit time periods
#
...
ggsave(filename = paste0("accumu_CP_Sept1March31_85.pdf"), 
...
ggsave(filename = paste0("accumu_CP_Sept1March31_45_mean_of_models_median_of_years.pdf"), 
       plot=mean_of_models, 
       width=plt_width, height=plt_height, units="in", 
       dpi = qual, device="pdf", 
       path=plot_base)

ggsave(filename = paste0("accumu_CP_Sept1March31_45_median_of_models_median_of_years.pdf"), 
       plot=median_of_models, 
       width=plt_width, height=plt_height, units="in", 
       dpi = qual, device="pdf", 
       path=plot_base)
