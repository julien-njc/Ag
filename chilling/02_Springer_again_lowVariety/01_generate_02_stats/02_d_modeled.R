...
if(hist){
  ...
  for(i in 1:length(the_dir)){
    ...
    data_list_hist[[i]] <-  threshold_func_Springer(file, data_type="modeled")
    rm(file)
  }

  # 5b. Process gathered historical data ------------------------------
  # Get medians for each location during historical period
  summary_data_historical <- get_medians_springer(data_list_hist)
  ...
  write.table(x = data_historical,
              file = file.path(main_out,
                               paste0("summary_springer_lowVariety_",
                     # model name
  ...
  write.table(x=summary_data_historical,
              file = file.path(out, paste0("/summary_stats_springer_lowVariety_",
                     # model name
                     gsub("-", "_",basename(dirname(getwd()))),
     ...
     for(i in 1:length(the_dir)){
         file <- read.table(file = the_dir[i], header = T,
                          colClasses = c("factor", "numeric", 
                                         "numeric", "numeric",
                                         "numeric", "numeric"))
         print ("Line 151 of modeled")
         print (names(data_list_F))
         print (the_dir[i])
         names(data_list_F)[i] <- the_dir[i]
         file <- na.omit(file)
         data_list_F[[i]] <- process_data_non_overlap_springer(file, time_period="doesNotMatter")
         rm(file)
     }
  ...
  summary_data_F <- get_medians_springer(data_list_F)
  print("line 148")
  ...
  # Grab lat/long
  summary_data_F <- grab_coord_springer(summary_data_F)
