...
heat_cut <- 25
heat_detla <- 1
min_DD_forRange <- heat_cut - 1
max_DD_forRange <- heat_cut + 1
...
vertDD_quan_25_100$y_intercept <- heat_cut
...
ggsave(plot = merged_plt,
       filename = paste0("Acum_CPHeat_", gsub(" ", "_", app_tp), "_RCP45_Kirti2.png"), 
...
ggsave(plot = merged_plt,
       filename = paste0("Acum_CPHeat_", gsub(" ", "_", app_tp), "_RCP85_Kirti2.png"), 
       width = 13, height=5, units = "in", 
       dpi = 400, device = "png",
       path = plot_dir)
print (plot_dir)
