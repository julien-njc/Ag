...
daily_CP <- rbind(daily_CP_RCP85, daily_CP_RCP85)
...
daily_CP <- subset(daily_CP, select = c(needed_cols, "cume_portions"))
daily_vertDD <- subset(daily_vertDD, select = c(needed_cols, "vert_Cum_dd"))

all_data <- dplyr::left_join(x = daily_CP, 
                             y = daily_vertDD, 
                             by = c("location", "model", "emission", "time_period")
                             )
...
############################################################
##
##  pick up needed column so that melts work correctly.å
##
##
needed_cols <- c("location", "emission", "model", "time_period", "chill_dayofyear")
...
ggsave(plot = merged_plt,
       filename = paste0("Acum_CPHeat_", gsub(" ", "_", app_tp), "_RCP45_", "2rows.png"), 
...
ggsave(plot = merged_plt,
       filename = paste0("Acum_CPHeat_", gsub(" ", "_", app_tp), "_RCP85_", "2rows.png"), 
       width = 13, height=10, units = "in", 
       dpi = 400, device = "png",
       path = plot_dir)
print (plot_dir)
