...
#############################################################
###
###               Read data/parameters off the disk
###
#############################################################
########
########  parameters
...
#____________________________________________________________
#
# Pick the days between Feb 15 and April 15 
# 

daily_vertDD <- daily_vertDD %>% 
                filter(chill_dayofyear >= 168) %>% 
                filter(chill_dayofyear <= 227) %>% 
                data.table()
...
eugene_85_F3 <- daily_vertDD %>% filter(location == "Eugene" & emission == "RCP 8.5" & time_period=="2076-2099") %>% data.table()
eugene_85_F3[, list(intercept = coef(lm(median_over_model_yr ~ chill_dayofyear))[1], 
                    coef      = coef(lm(median_over_model_yr ~ chill_dayofyear))[2]),
                 by = c("emission", "time_period", "location")]


saveRDS(A, paste0(daily_vertDD_dir, "slopes_maxDD_CalendarBased_HV.rds"))
